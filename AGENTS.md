...
`jonas-thelemann_stack` is the Docker Swarm configuration for [jonas-thelemann.de](https://jonas-thelemann.de/), managed with [dargstack](https://github.com/dargstack/dargstack/).
...
- Use natural language in any non-code text instead of referring to code directly, e.g. "the database's password" instead of "the `postgres-password`", except when a code reference is needed
...
- Git commit scopes must be chosen as follows (ordered by priority):
  1. service name, e.g. `postgres`, `traefik`, `jonas-thelemann`
